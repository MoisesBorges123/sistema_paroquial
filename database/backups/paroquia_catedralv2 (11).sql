...
--
-- Host: 127.0.0.1
-- Generation Time: 10-Fev-2020 às 11:29
...
(56, 'Natália', 12, '1997-11-06', 'moisesalvesborges@gmai.com', 3, NULL),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `meus_dizimistas`  AS  select `dizimistas`.`id_dizimista` AS `id_dizimista`,`dizimistas`.`pessoa` AS `pessoa`,`dizimistas`.`d_cadastro` AS `d_cadastro`,`dizimistas`.`situacao` AS `situacao`,`pessoas`.`id_pessoa` AS `id_pessoa`,`pessoas`.`nome` AS `nome`,`pessoas`.`endereco` AS `endereco`,`pessoas`.`d_nasc` AS `d_nasc`,`pessoas`.`email` AS `email`,`pessoas`.`sexo` AS `sexo`,`enderecos`.`id_endereco` AS `id_endereco`,`enderecos`.`logradouro` AS `logradouro`,`enderecos`.`num_casa` AS `num_casa`,`enderecos`.`apartamento` AS `apartamento`,`enderecos`.`complemento` AS `complemento`,`enderecos`.`observacoes` AS `observacoes`,`logradouros`.`id_logradouro` AS `id_logradouro`,`logradouros`.`rua` AS `rua`,`logradouros`.`bairro` AS `bairro`,`logradouros`.`cep` AS `cep`,`logradouros`.`cidade` AS `cidade`,`logradouros`.`estado` AS `estado`,`estado`.`id_estado` AS `id_estado`,`estado`.`nome_estado` AS `nome_estado`,`estado`.`sigla` AS `sigla` from ((((`dizimistas` join `pessoas`) join `enderecos`) join `logradouros`) join `estado`) where ((`dizimistas`.`pessoa` = `pessoas`.`id_pessoa`) and (`pessoas`.`endereco` = `enderecos`.`id_endereco`) and (`enderecos`.`logradouro` = `logradouros`.`id_logradouro`) and (`logradouros`.`estado` = `estado`.`id_estado`)) ;
